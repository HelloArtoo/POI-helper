...
public class ExcelUtil {
    ...
    /**
     * 导出一个07 excel 此方法只支持一个sheet 07以上的数据格式，支持大数据导出且避免内存溢出
     * 
     * @param sheetName
     *            sheet的名称
     * @param titles
     *            标题栏每列对应的名称 （不能为空）
     * @param cellWidth
     *            每列对应的单元格宽度 （不能为空） 如 2000 , 6000, 12000...
     * @param dataArgNames
     *            与data对应，表示map中数据对应在哪一列进行呈现(与map中数据要对应 （不能为空）
     * @param data
     *            数据源，集合 （不能为空）
     * @throws Exception
     */
    public static SXSSFWorkbook exportStandard07ExcelFromCollection(String sheetName, String[] titlesArr,
                                                                    int[] cellWidthArr, String[] dataArgNamesArr,
                                                                    List<Map<String, Object>> data) throws Exception {

        if (titlesArr == null || cellWidthArr == null || dataArgNamesArr == null
            || titlesArr.length != cellWidthArr.length || titlesArr.length != dataArgNamesArr.length) {
        ...
        for (Map<String, Object> map : data) {
            ...
            for (int i = 0; i < dataArgNamesArr.length; i++) {
                cell = row.createCell(i);
                cell.setCellValue(map.get(dataArgNamesArr[i]) == null ? ""
                    : String.valueOf(map.get(dataArgNamesArr[i])));
            }
        }
    ...
    /**
     * 导出一个03 excel 此方法只支持一个sheet 03数据格式
     * 
     * @param sheetName
     *            sheet的名称
     * @param titles
     ...
     * @throws Exception
     */
    public static HSSFWorkbook exportStandard03ExcelFromCollection(String sheetName, String[] titlesArr,
                                                                   int[] cellWidthArr, String[] dataArgNamesArr,
                                                                   List<Map<String, Object>> data) throws Exception {
    ...
    /**
     * 读取03或者07的excel数据，返回list集合 问：XSSFWorkbook貌似比HSSFWorkbook慢很多
     * 
     * @param file
     *            excel文件
     * @param dataArgNamesArr
     *            用来保存每列在map中的英文表示
     * @return
     * @throws Exception
     */
    public static List<Map<String, Object>> readExcel4Collection(File file, String[] dataArgNamesArr) throws Exception {
        ...
        while (rows.hasNext()) {
            ...
            while (cellBody.hasNext()) {
                ...
                map.put(dataArgNamesArr[i++], cell.getStringCellValue() == null ? null : cell.getStringCellValue()
                    .trim());
            }
            result.add(map);
            i = 0;
